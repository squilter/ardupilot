...
#define SYM_SIDEBAR_R_ARROW 0x09
#define SYM_SIDEBAR_L_ARROW 0x0A
#define SYM_SIDEBAR_A 0x13
#define SYM_SIDEBAR_B 0x14
#define SYM_SIDEBAR_C 0x15
#define SYM_SIDEBAR_D 0xDD
#define SYM_SIDEBAR_E 0xDB
#define SYM_SIDEBAR_F 0xDC
#define SYM_SIDEBAR_G 0xDA
#define SYM_SIDEBAR_H 0xDE
#define SYM_SIDEBAR_I 0x11
#define SYM_SIDEBAR_J 0x12
...
void AP_OSD_Screen::draw_sidebars(uint8_t x, uint8_t y)
{
    ...
    for (int i = 0; i < 7; i++){
        if (i == 3) {
            ...
            backend->write(x, y+i, false, "%3d%c%c", (int) scaled_aspd, u_icon(SPEED), SYM_SIDEBAR_R_ARROW);
        } else {
            backend->write(x+4, y+i, false,  "%c", sidebar_sectors[aspd_symbol_index]);
        }
    ...
    for (int i = 0; i < 7; i++){
        if (i == 3) {
            ...
            backend->write(x+16, y+i, false, "%c%d%c", SYM_SIDEBAR_L_ARROW, (int) scaled_alt, u_icon(ALTITUDE));
        } else {
            backend->write(x+16, y+i, false,  "%c", sidebar_sectors[alt_symbol_index]);
        }
